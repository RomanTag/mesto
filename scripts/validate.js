...
const setEventListeners = (formToValidate, { inputSelector, submitButtonSelector, ...rest }, handleEditFormSubmit) => {
  ...
  formInputs.forEach(input => {
    input.addEventListener('input', () => {
      checkInputValidity(input, formToValidate)
...
const checkInputValidity = (input, { inputErrorClass, ...rest }) => {
  ...
  if (input.checkValidity()) {
    ...
    containerEl.classList.remove('popup__input-container_invalid');
  } else {
    currentInputErrorContainer.textContent = input.validationMessage
    const containerEl = currentInputErrorContainer.closest('.popup__input-container');
    containerEl.classList.add('popup__input-container_invalid');
  }
}
